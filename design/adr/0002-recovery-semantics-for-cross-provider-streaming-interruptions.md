# ADR-0002: Recovery semantics for cross-provider streaming interruptions
...
When SSE switches to the downgrade model midway, how to deal with the delivered fragments (truncation/resumption)? The client contract needs to be clear.
...
This ADR solidifies the "recovery semantics of cross-provider streaming interruption" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
